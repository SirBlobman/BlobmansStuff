...
public final class BlueSlimeOuterLayer extends RenderLayer<BlueSlime, BlueSlimeModel> {
    private final BlueSlimeModel model;

    public BlueSlimeOuterLayer(RenderLayerParent<BlueSlime, BlueSlimeModel> parent, EntityModelSet modelSet) {
        super(parent);
        ModelPart modelPart = modelSet.bakeLayer(BSModelLayers.BLUE_SLIME_OUTER);
        this.model = new BlueSlimeModel(modelPart);
    }

    @Override
    public void render(@NotNull PoseStack poseStack, @NotNull MultiBufferSource bufferSource, int p_117472_,
                       @NotNull BlueSlime entity, float p_117474_, float p_117475_, float p_117476_,
        ...
        if (!invisible || glowing) {
            ...
            BlueSlimeModel parentModel = getParentModel();
